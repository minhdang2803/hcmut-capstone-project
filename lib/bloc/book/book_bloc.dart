...
class BookListBloc extends Bloc<BookListEvent, BookListState> {
  ...
  void _onLoadByCategory(
      LoadByCategoryEvent event, Emitter<BookListState> emit) async {
    try {
...
class BookBloc extends Bloc<BookEvent, BookState> {
  ...
  void _onLoadEbook(LoadEbookEvent event, Emitter<BookState> emit) async {
    ...
    try {
      ...
      LogUtil.error('Get Ebook error ${e.message}', error: e, stackTrace: s);
    } catch (e) {
      emit(BookErrorState(e.toString()));
    }
  }

  void _onLoadEbookAnother(
      LoadEbookAnotherEvent event, Emitter<BookState> emit) async {
    emit(BookLoadingState());
    try {
      final response = await _bookRepos.getEbook(event.bookId, event.pageKey);
    ...
    void _onLoadAudioBook(
      LoadAudioBookEvent event, Emitter<BookState> emit) async {
    emit(BookLoadingState());
    try {
  ...
  void _onRemoveFavorite(
      RemoveFavoriteEvent event, Emitter<BookState> emit) async {
