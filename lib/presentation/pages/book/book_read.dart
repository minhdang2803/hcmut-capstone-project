...
class _BookReadState extends State<BookRead> {
  ...
  void _scrollListener() async {
    ...
    if (_maxExtent == 0.0) {
      ...
      _scrollControllerBottomUp = ScrollController(
          initialScrollOffset:
              _maxExtent); //init a new scroll controller that set initial offset to end
    ...
    if ((_scrollController.offset - _oldCkpt).abs() > 1000 ||
      ...
      final int savedIdx = (_scrollController.offset / _rowHeight +
              Constants.defaultReadingPageSize * (_pageNumber - 1))
          .toInt();
  ...
  void _scrollListenerBottomUp() async {
    ...
    if ((_scrollControllerBottomUp.offset - _oldCkpt).abs() > 1000 ||
      ...
      _oldPage = _pageNumber;
      final int savedIdx = (_scrollControllerBottomUp.offset / _rowHeight +
              Constants.defaultReadingPageSize * (_pageNumber - 1))
          .toInt();
      // LogUtil.debug('$_oldCkpt and $savedIdx');
      _bookBloc.add(UpdateCkptEvent(
